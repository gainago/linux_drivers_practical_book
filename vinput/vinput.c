...
static struct cdev vinput_cdev;
static dev_t dev;
/* Для доступа к глобальным данным устройства. */
...
static struct vinput_device *vinput_get_device_by_type(const char *type)
{
    ...
    list_for_each (curr, &vinput_devices) {
        ...
        if (vinput && strncmp(type, vinput->name, strlen(vinput->name))
                     == 0) {
            found = 1;
            break;
        }
...
/* Поиск ID виртуального устройства в свяанном списке vinput_vdevices,
...
static ssize_t vinput_write(struct file *file, char __user *buffer,
                            size_t count, loff_t *offset)
{
    char buff[VINPUT_MAX_LEN + 1];
    struct vinput *vinput = file->private_data;
                            ...
                            const char *buf, size_t len)
{
    ...
    if (IS_ERR(device)) {
        pr_info("vinput: This virtual device is not registered\n");
        err = PTR_ERR(device);
        goto fail;
    }
...
/* Этот макрос генерирует структуру class_attr_expotr and export_store() */
...
/* Этот макрос генерирует структуру class_attr_unexpotr 
 * и unexpotr_store().
 */
...
static int __init vinput_init(void)
{
    ...
    err = alloc_chrdev_region(&dev, 0, VINPUT_MAX_LEN, DRIVER_NAME);
    if (err < 0) {
        pr_err("Could not alloc_chrdev_region");
        goto failed_alloc;
    }

    cdev_init(&vinput_cdev, &vinput_fops);

    err = cdev_add(&vinput_cdev, dev, VINPUT_MINORS);
    if (err < 0) {
        pr_err("Unable add chrdev\n");
        goto fail_cdev_add;
    }
    ...
    if (err < 0) {
        pr_err("Unable to register vinput class\n");
        goto failed_class;
    }
    return 0;
failed_class:
    cdev_del(&vinput_cdev);
fail_cdev_add:
    unregister_chrdev_region(dev, VINPUT_MINORS);
failed_alloc:
    return err;
}
